...
/**
 * Mengambil data komentar dari Google Apps Script
 */
export async function fetchKomentarData() {
  const response = await fetch(`${GAS_WEBAPP_URL}?action=getKomentarData`, {
    method: 'GET',
    redirect: 'follow',
  });
  return response.json();
...
/**
 * Menyimpan data komentar ke Google Apps Script
 */
export async function saveKomentarData(payloadData) {
  const response = await fetch(GAS_WEBAPP_URL, {
    method: 'POST',
    headers: { 'Content-Type': 'text/plain;charset=utf-8' },
    redirect: 'follow',
    body: JSON.stringify({ action: 'saveKomentarData', data: payloadData }),
  });
  return response.json();
}
